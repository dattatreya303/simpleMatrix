...
BOOST_AUTO_TEST_CASE_TEMPLATE( test_valid_object, T, test_constructor_types ) {
    ...
    for (size_t i=0; i<num_rows; i++) {
        for (size_t j=0; j<num_cols; j++) {
            BOOST_REQUIRE_MESSAGE( abs(test_matrix(i,j) - (i+j)/(T)(2.0)) <= 1e-9, "// All values should be same as intended with a tolerance of 1e-9 for floats and doubles." );
        }
    }
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE( test_addition, T, test_constructor_types ) {
    ...
    for (size_t i=0; i<num_rows; i++) {
        for (size_t j=0; j<num_cols; j++) {
            ...
            BOOST_REQUIRE_MESSAGE( abs(c(i,j) - val) <= 1e-6 && abs(c(i,j) - val) <= 1e-6, "// Every element should be correctly calculated with a tolerance of 1e-6 for floats and doubles." );
        }
    }
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE( test_addition_complex, T, test_constructor_types ) {
    ...
    for (size_t i=0; i<num_rows; i++) {
        for (size_t j=0; j<num_cols; j++) {
            ...
            BOOST_REQUIRE_MESSAGE( abs(c(i,j).real() - val.real()) <= 1e-6 && abs(c(i,j).imag() - val.imag()) <= 1e-6, "// Every element should be correctly calculated with a tolerance of 1e-6 for floats and doubles." );
        }
    }
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE( test_multiplication, T, test_constructor_types ) {
    ...
    for (size_t i=0; i<num_rows1; i++) {
        for (size_t j=0; j<num_cols2; j++) {
            ...
            BOOST_REQUIRE_MESSAGE( abs(c(i,j) - val) <= 1e-6, "// Every element should be correctly calculated with a tolerance of 1e-6 for floats and doubles." );
        }
    }
}